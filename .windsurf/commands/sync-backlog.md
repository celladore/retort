...
Route issues to teams by area: `backend`→backend, `frontend`→frontend, `data`→data, `infra`→infra, `devops`→devops, `testing`→testing, `security`→security, `docs`→docs, `product`→product, `quality`→quality, `cli`→backend, `sync-engine`→devops, `forge`→forge
