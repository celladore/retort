...
2. Run `pnpm --dir .agentkit retort:sync`
...
5. **Never** directly edit files marked `<!-- GENERATED by Retort — DO NOT EDIT -->` — modify the spec in `.agentkit/spec/` and run `retort sync` instead; if spec files changed, run `pnpm --dir .agentkit retort:sync` and commit regenerated outputs before creating a PR
