...
- **State file:** `.agents/state/orchestrator.json` — tracks phase, teams, metrics, risks, retry policy
- **Events log:** `.agents/state/events.log` — append-only audit trail
- **Task files:** `.agents/state/tasks/` — per-task JSON with lifecycle: submitted → accepted → working → completed/failed/rejected
