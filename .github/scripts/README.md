...
Configures branch protection rules for `main` using the GitHub CLI.
...
Run once after initial `retort sync`. The script is idempotent — running
...
using the Retort merge resolution matrix. Files requiring semantic understanding
...
```bash
# Resolve conflicts with default branch (main)
