# SPEC-004: Organizational Framework Practices

**Status**: Draft
**Phase**: Cross-phase — items span Tiers 1-3 from the organizational frameworks plan
**Scope**: Themes A (Flow), B (Quality), C (Decision-Making), D (Team Structure), E (Planning), F (Learning), G (Sustainability)
...
This spec covers 7 themes with ~50 practices sourced from Kanban, XP, Lean, Shape Up, Team Topologies, DORA, TPS, TOC, ICS, Mission Command, OKRs, Cynefin, and Six Sigma. The same rule applies: **if you can enforce it in code, don't describe it in agent instructions**.

Many items here overlap with or extend SPEC-001/002/003 features. Where overlap exists, this spec adds the **incremental delta** only — it does not re-specify what's already covered.
...
**TR-057.1 — Already spec'd**: Sprint buffer is defined in SPEC-001 (TR-005.3):
```yaml
process:
...
_See also: SPEC-001, SPEC-002, SPEC-003 for companion specifications._
