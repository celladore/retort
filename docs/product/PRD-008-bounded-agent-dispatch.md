...
- Distinguish isolated investigation, recent-context work, and durable continuation.
...
- A dispatch may narrow its declared context at invocation time. Broadening requires an explicit
  caller decision and a recorded reason.
...
Artifacts are referenced by stable URI or repository-relative path and may include a digest,
media type, classification, and purpose. Dispatchers validate access before launch. Artifact
contents are loaded by the child only when needed.
...
- `dispatchId` and `parentDispatchId`;
- `taskId` and `runId`;
- `traceId`;
- agent identity;
...
or metrics. Objectives, paths, prompts, artifact content, personal data, credentials, and free-form
...
- No task text, personal data, credentials, or artifact content appears in gateway metric labels.
