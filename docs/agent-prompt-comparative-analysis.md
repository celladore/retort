...
| **2** | Strong | Uses `## Role` and `## Responsibilities` headers to the same effect. Adds a meta-instruction ("Begin with a concise checklist of 3-7 bullets") that clarifies *how* the agent should start working, not just *what* it should do. |
...
| **3** | Weak | Lists responsibilities but provides no decision-making framework. The agent knows *what* it can do but not *when* to choose one action over another or *when* to stop and escalate. |
