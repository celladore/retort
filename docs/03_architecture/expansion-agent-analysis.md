...
The gap is real. The repository has excellent *execution* infrastructure (orchestrator, task protocol, teams, review gates) but lacks a *strategic analysis* layer that generates well-reasoned suggestions about what to build, document, or improve. The expansion agent fills this gap.
...
- For each *approved* suggestion, generate appropriate specification artifacts
...
- Auto-generating *draft* specs from analysis reduces the manual overhead of documentation
...
- Generated ADRs risk losing the *actual* decision context that only humans possess
...
3. **Be cautious with PRDs** — PRDs define *what to build*, which requires product judgment the agent doesn't have. Generate PRD *templates* with questions, not PRDs with answers.
