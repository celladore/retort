...
Read the canonical brand spec at `.agentkit/spec/brand.yaml` and validate it: check that identity.name exists, colors.primary.brand is a valid hex (either a plain string like `"#1976D2"` or an object with a `.hex` field), semantic colors are complete, and dark mode colors are present. Summarise the brand identity (name, primary color, attributes) and flag any validation warnings.

Then search for design assets, style guides, theme files, and UI libraries. Cross-reference discovered design tokens against `.agentkit/spec/brand.yaml` for consistency — flag any hardcoded hex values that diverge from the brand palette.
...
For each category below, find **1-10 high-impact items** with: ID, title, severity (critical/high/medium/low), effort (S/M/L), area (from canonical list), location, description, impact, and recommendation.
...
3. **Design System Summary** (from Phase 0.5) — include brand.yaml validation status and palette overview
...
During Phase 1c analysis, any finding that targets a **generated file** (contains `<!-- GENERATED by Retort`) or an **Retort template** (`.agentkit/templates/**`) must be filed as an external issue if its severity is **critical** or **high**. File the issue immediately when you identify it — do not defer to Phase 3.

### Canonical Field Values

When filing issues (GitHub or Linear), use these **exact** values from the project's issue template enums:

- **Severity:** `critical`, `high`, `medium`, `low` (lowercase)
- **Priority:** `P0`, `P1`, `P2`, `P3`, `P4` — map critical→P0, high→P1, medium→P2, low→P3
...
- `.agentkit/templates/**`, `docs/**` → `docs`
- `auth/**`, `security/**` → `security`
- `infra/**`, `terraform/**` → `infra`
- Default for `.agentkit/**` → `sync-engine`
...
<severity> — <description from issue template>
...
BODY
  ...
  --label "agentkit,generated-output,severity:<severity>,area:<area>,priority:<p-level>"
```
...
linear issue create \
  ...
  --label "agentkit,area:<area>,severity:<severity>"
```
