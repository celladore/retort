...
Use canonical lowercase severity values matching the issue template and task-protocol:
...
For unresolved issues with severity >= `high`, file in the project's configured tracker. Follow the same issue-filing protocol as the standard review mode (see "Template & Generated-Format Issue Filing" above), using labels `retrospective,session-issue,severity:<severity>,area:<area>`.
