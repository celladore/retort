...
<!-- Manually authored — not generated by agentkit sync. Safe to edit directly. -->
...
| Input | Behaviour |
| --- | --- |
| _(empty)_ | Run full detection → status → guided choices |
| A task description (e.g., `I want to add auth`) | Skip status, route directly to the right team/command |

No flags are supported. Any text in `$ARGUMENTS` is treated as a task description.
...
This command is **read-only**. It detects state but does not modify it.

### Reads

| File | Purpose |
| --- | --- |
| `.agentkit/` | Check if AgentKit Forge is initialised |
| `.claude/commands/orchestrate.md` | Check if sync has been run |
| `AGENT_TEAMS.md` | Check if discovery has been completed |
| `AGENT_BACKLOG.md` | Count backlog items |
| `.claude/state/orchestrator.json` | Read session phase and current task |
| `.claude/state/tasks/*.json` | Count active tasks |
| `.claude/state/orchestrator.lock` | Check if a session lock is held |
| `.claude/state/events.log` | Check for recent activity |

### Writes

None. No files are created, modified, or deleted. No events are logged.
