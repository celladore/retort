...
Always use `--base main` when creating pull requests in **retort**.

## Rule

**Event**: bash
**Action**: warn
**Pattern**: `gh pr create` without `--base main`
...
- `main` — integration branch, the correct PR target for all feature work

Creating PRs against `main` instead of `main` triggers an automatic reverse-merge PR (merging `main` back into `main`), which creates noise and bypasses the integration pipeline.
