...
```bash
{{packageManager}} -C .agentkit agentkit:sync
```
...
2. Run `{{packageManager}} -C .agentkit agentkit:sync`
...
5. **Never** directly edit files marked `<!-- GENERATED by AgentKit Forge — DO NOT EDIT -->` — modify the spec in `.agentkit/spec/` and run `agentkit sync` instead; if spec files changed, run `{{packageManager}} -C .agentkit agentkit:sync` and commit regenerated outputs before creating a PR
