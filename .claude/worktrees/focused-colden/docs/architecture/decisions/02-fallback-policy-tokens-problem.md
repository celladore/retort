...
The `retort` project uses one or more evidence-driven scoring or gating metrics (for example: cost evidence, telemetry confidence, quality signal confidence). In some workflows, required evidence can be missing at decision time.

Baseline source for this template: the current fallback ADR from `retort`.
...
Applies to `/infra-eval` scoring dimensions, `/review` quality gates, and related evaluation workflows in `retort`.
