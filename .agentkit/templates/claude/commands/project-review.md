...
During Phase 1c analysis, any finding that targets a **generated file** (contains `<!-- GENERATED by Retort`) or a **Retort template** (`.agentkit/templates/**`) must be filed as an external issue if its severity is **critical** or **high**. File the issue immediately when you identify it — do not defer to Phase 3.
