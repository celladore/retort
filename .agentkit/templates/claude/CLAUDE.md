...
{{/if}}{{#if hasSessionHandoff}}/handoff                  → Document session for continuity
{{/if}}```
{{/if}}
...
5. **Never** directly edit files marked `<!-- GENERATED by AgentKit Forge — DO NOT EDIT -->` — modify the spec in `.agentkit/spec/` and run `agentkit sync` instead
6. **Always** run `pnpm -C .agentkit agentkit:sync` after modifying any `.agentkit/spec/` file
7. **Always** use Conventional Commits format for commit messages AND PR titles
8. **Always** run `/check` before creating a PR
9. **Always** document breaking changes in ADRs
