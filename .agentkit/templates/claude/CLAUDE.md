...
4. **Never** modify files in `.agentkit/templates/`, `.agentkit/spec/`, `.agentkit/engines/`, `.agentkit/overlays/`, or `.agentkit/bin/` — these are the upstream source-of-truth for AgentKit Forge and are protected by a PreToolUse hook
5. **Never** directly edit files marked `<!-- GENERATED by AgentKit Forge — DO NOT EDIT -->` — modify the spec and run `agentkit sync` instead
6. **Always** run `/check` before creating a PR
7. **Always** document breaking changes in ADRs
