...
export function getGeneratedHeader(version, repoName, ext, vars = {}) {
  ...
  const syncCmd =
    pm === 'npm'
      ? 'npm run --prefix .agentkit agentkit:sync'
      : `${pm} --dir .agentkit agentkit:sync`;
  return [
...
export function insertHeader(content, ext, version, repoName) {
  ...
  // Only check the first 500 chars so body text that MENTIONS "GENERATED by AgentKit Forge"
  // (e.g., in rule docs explaining the protection policy) does not trigger infinite recursion.
  if (content.slice(0, 500).includes('GENERATED by AgentKit Forge')) {
    // Strip all contiguous comment lines at the start (the old header block)
    ...
    const bodyWithoutLegacyHeader = lines.slice(i).join('\n').replace(/^\n+/, '');
    // Re-run insertHeader on the stripped content to get the correct insertion point.
    // After stripping the header block the body cannot start with a legacy header comment,
    // so this recursive call will not recurse again.
    return insertHeader(bodyWithoutLegacyHeader, ext, version, repoName);
  }
