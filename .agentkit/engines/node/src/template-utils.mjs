...
export function flattenProjectYaml(project, docsSpec = null) {
  ...
  vars.pmInstall = pm === 'yarn' ? 'yarn install --frozen-lockfile' : pm === 'npm' ? 'npm ci' : 'pnpm install --frozen-lockfile';
  vars.pmRun = pm === 'yarn' ? 'yarn' : pm === 'npm' ? 'npm run' : 'pnpm';
  vars.pmExec = pm === 'yarn' ? 'yarn' : pm === 'npm' ? 'npx' : 'pnpm';
  vars.pmLockfile = pm === 'yarn' ? 'yarn.lock' : pm === 'npm' ? 'package-lock.json' : 'pnpm-lock.yaml';
...
export function getGeneratedHeader(version, repoName, ext, vars = {}) {
  ...
  const syncCmd = pm === 'npm' ? 'npm run -C .agentkit agentkit:sync' : `${pm} -C .agentkit agentkit:sync`;
  return [
