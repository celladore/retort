...
export function insertHeader(content, ext, version, repoName) {
  ...
  if (content.includes('GENERATED by AgentKit Forge')) {
    ...
    // Shebang lines (#!) must be preserved and skipped before comment stripping —
    // they start with '#!' not '# ', so the loop would stop at i=0, leaving
    // content unchanged and causing infinite recursion on the recursive call.
