...
// ---------------------------------------------------------------------------
// Tests: Sync Integration — Dynamic language instructions (Option B)
// ---------------------------------------------------------------------------
describe('syncCopilotLanguageInstructions — dynamic generation from rules.yaml (via runSync --only copilot)', () => {
  let projectRoot;
  ...
  it('generates languages/ subdirectory under .github/instructions/', { timeout: 15000 }, () => {
    expect(
      existsSync(resolve(projectRoot, '.github', 'instructions', 'languages'))
    ).toBe(true);
  });

  it('generates one file per rules.yaml domain in languages/', { timeout: 15000 }, () => {
    const files = collectFiles(resolve(projectRoot, '.github', 'instructions', 'languages'));
    // rules.yaml has: typescript, dotnet, python, rust, iac, security, blockchain, testing
  ...
  it('generates languages/README.md', () => {
    expect(
  ...
  it('domain-specific template is used for typescript.md', () => {
    ...
    // Domain-specific template has richer TS-specific content
    expect(content).toContain('TypeScript');
    expect(content).toContain('GENERATED by AgentKit Forge');
  });
  ...
  it('ruleConventions from rules.yaml are injected into domain files', () => {
    ...
    // typescript.md template exposes Project Conventions section when ruleConventions is set
  ...
  it('security.md uses generic TEMPLATE.md fallback (no domain-specific template)', () => {
    ...
    // Generic TEMPLATE.md renders Conventions heading + ruleConventions
    expect(content).toContain('Conventions');
    expect(content).toContain('GENERATED by AgentKit Forge');
  });

  it('no raw Handlebars placeholders remain in generated files', () => {
  ...
  it('languages/README.md does not contain unresolved language placeholders', () => {
    const content = readFileSync(
